...
let nomeUsuario = document.getElementById("nomeUsuario")as HTMLInputElement;
let senha = document.getElementById("senha") as HTMLInputElement;
let confirmaSenha = document.getElementById("confirmaSenha") as HTMLInputElement;
...
function checkImputs(){
  ...
  let usuarios:Array<User> = JSON.parse(localStorage.getItem("usuarios")|| '[]') ;
  ...
  let sucessoComfirm = document.getElementById("sucessos") as HTMLDivElement;
...
function setErrorFor(input:HTMLInputElement, message:string) {
  ...
  const small = formControl.querySelector("small") as HTMLSpanElement;
  small.innerText = message;
  formControl.className = "formControl erro";
}

function setSucessoFor(input:HTMLInputElement) {
  const formControl = input.parentElement as HTMLDivElement;
